...
function setList(xml) {
    ...
    const collection = xmlDoc.getElementsByTagName("url");
    var allURLs = [];
    for (let c of collection) {
        const url = c.getElementsByTagName("loc")[0].childNodes[0].nodeValue;
        allURLs.push(url);
    }
    allURLs.sort();
    var dirOld = dir(allURLs[0]);
    for (const url of allURLs) {
        let sitename = url.split("/");
...
function sortProjectList() {
    ...
    while (switching) {
        ...
        for (i = 0; i < b.length - 1; i++) {
            shouldSwitch = false;
            if (
                b[i].firstElementChild.innerHTML.toLowerCase() >
                b[i + 1].firstElementChild.innerHTML.toLowerCase()
            ) {
                shouldSwitch = true;
                break;
            }
